...
import { commands, getHelpText, getStartText } from './messages';
...
const botCommands = commands.map((c) => ({
  command: c.command,
...
bot.onText(/\/start/, (msg) => {
  ...
  storeChatId(chatId);
  bot.sendMessage(chatId, getStartText());
});
...
bot.onText(/\/add (.+)/, async (msg, match) => {
  ...
  const [username, realName] = match[1].split(' ');
...
bot.onText(/\/edit (.+)/, (msg, match) => {
  ...
  bot.sendMessage(
    ...
    `Редагування користувача ${username} (${user.realName}).\n` +
      "Введіть новий нікнейм та ім'я в форматі: {username} {real name}"
  );
});
...
bot.onText(/\/full_stats(?:\s+(.+))?/, async (msg, match) => {
  const chatId = msg.chat.id;
  storeChatId(chatId);

  if (match && match[1]) {
    const username = match[1];
    await sendUserFullStats(chatId, username);
  } else {
    await sendAllUsersFullStats(chatId);
  }
});
...
async function sendUserFullStats(
  chatId: number,
  username: string
): Promise<void> {
  const users = db.getUsers();
...
async function sendAllUsersFullStats(chatId: number): Promise<void> {
  const users = db.getUsers();
  ...
  bot.sendMessage(
    chatId,
    'Отримання повної статистики для всіх користувачів...'
  );

  const statsPromises = users.map(async (user) => {
    const userData = await lichessService.getUserData(user.username);
    return userData ? formatFullStats(userData, user) : null;
  });
  ...
  if (validStats.length === 0) {
    bot.sendMessage(
      chatId,
      'Не вдалося отримати повну статистику для жодного користувача'
    );
    return;
  }

  // Надсилаємо одне велике повідомлення
  const message = validStats.join('\n\n');
  bot.sendMessage(chatId, message);
}
